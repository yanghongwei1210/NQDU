...
class NetworkEmbedding(nn.Module):
    ...
    @staticmethod
    def net_pro_loss(emb, a,sample_weight=None):
        r = torch.sum(emb*emb, 1)
        r = torch.reshape(r, (-1, 1))
        dis = r-2*torch.matmul(emb, emb.T)+r.T
        if sample_weight is not None:
            # return torch.mean(torch.mul(torch.sum(torch.FloatTensor(a).__mul__(dis), 1),sample_weight))
            return torch.mean(torch.mul(torch.sum(torch.FloatTensor(a).to('cuda').__mul__(dis), 1), sample_weight))
        else:
            # return torch.mean(torch.sum(torch.FloatTensor(a).__mul__(dis), 1))
            return torch.mean(torch.sum(torch.FloatTensor(a).to('cuda').__mul__(dis), 1))
