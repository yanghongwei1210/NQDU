...
from ACDNE import ACDNE_Trainer
from ACDNE import utils

class DVRL_ACDNE(object):
    def __init__(self, args, pred_model: ACDNE_Trainer, flags):
        self.args = args
        ...
        self.y_train_onehot=self.y_train
        self.y_valid_onehot=self.y_valid
    ...
    def train_dvrl(self, perf_metric,num):
        ...
        acc = utils.accuracy(y_predict,self.y_valid)
        micro,macro = utils.f1_scores(y_predict,self.y_valid)
        ...
        print(
            '*Final_model performance: num: {} \t accuracy: {:.4f}\t  micro-f1: {:.4f}\t  macro-f1: {:.4f} \t  modelpath: {}'.format(num,
    ...
    def dvrl_predictor(self, x_test):
        ...
        if self.flag_sgd:
            y_test_hat,hs,ht = self.final_model.predict(x_test)
        else:
            y_test_hat,hs,ht = self.final_model.predict(is_train_dataset=False, batch_size=self.batch_size_predictor, sample_weight=None, sample_indices=None)

        return y_test_hat,hs,ht